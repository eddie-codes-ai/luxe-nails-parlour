...
export default function ArtistsClient() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fd.append("file", file);
    const res = await fetch("/api/admin/gallery", { method: "PATCH", body: fd });
    const data = await res.json();
  ...
  const handleSave = async () => {
    ...
    const res = await fetch("/api/admin/artists", { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    const data = await res.json();
    if (data.success) {
  ...
  const handleDelete = async (id: number) => {
    ...
    setDeletingId(id);
    const res = await fetch("/api/admin/artists", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
    const data = await res.json();
