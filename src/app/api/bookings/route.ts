...
// POST /api/bookings
//
// Creates a new booking, locks the slot, starts the 1-hour expiry timer.
// For website bookings: status → pending_payment (slot locked, awaiting deposit)
// For late-night slots: status → pending_approval (no deposit yet, owner confirms first)
...
export async function POST(req: NextRequest) {
  ...
  if (!service_id || !booking_date || !start_time) {
    return NextResponse.json({ error: 'service_id, booking_date, and start_time are required' }, { status: 400 })
  }
  if (!['in_shop', 'house_call'].includes(location_type)) {
    return NextResponse.json({ error: 'location_type must be in_shop or house_call' }, { status: 400 })
  }
  if (location_type === 'house_call' && !house_call_address?.trim()) {
    return NextResponse.json({ error: 'house_call_address is required for house calls' }, { status: 400 })
  }
  ...
  try {
    ...
    if (location_type === 'house_call' && !service.house_call_available) {
      return NextResponse.json(
        { error: 'This service is not available as a house call' },
        { status: 400 }
      )
    }

    // ── Fetch artist (null = No Preference, owner assigns later) ─────────────────
    ...
    // ── Fetch artist schedule (skip if No Preference) ────────────────────────────
    ...
    if (artist_id) {
      ...
      if (schedule?.is_blocked) {
        return NextResponse.json(
          { error: 'Artist is not available on this date' },
          { status: 409 }
        )
      }
    }

    // ── Determine if this is a late-night slot ─────────────────────────────────
    const startMins        = timeToMins(start_time)
    const dayEndMins       = timeToMins(schedule?.end_time ?? '19:00')
    const lateCutoffMins   = schedule?.late_cutoff_time
                               ? timeToMins(schedule.late_cutoff_time)
                               : null
    const lateEndMins      = schedule?.late_end_time
                               ? timeToMins(schedule.late_end_time)
                               : null

    const isLateNight = lateCutoffMins !== null && startMins >= lateCutoffMins

    // Validate slot is within working hours
    const withinNormal = startMins >= timeToMins(schedule?.start_time ?? '09:30')
                      && startMins < dayEndMins
    const withinLate   = isLateNight && lateEndMins !== null
                      && startMins < lateEndMins

    if (!withinNormal && !withinLate) {
      return NextResponse.json(
        { error: 'Selected time is outside the artist\'s working hours' },
        { status: 400 }
      )
    }
    ...
    // ── Check slot is not already taken (skip if No Preference — slots API already checked) ──
    if (artist_id) {
    ...
    const lateNightFee    = isLateNight
                              ? Math.round(servicePrice * (lateNightSurcharge / 100) * 100) / 100
                              : 0
    ...
    // Late-night → pending_approval (owner must confirm before deposit is requested)
    // Normal     → pending_payment  (slot locked, customer pays deposit now)
    const initialStatus = isLateNight ? 'pending_approval' : 'pending_payment'

    // ── Set expiry ─────────────────────────────────────────────────────────────
    // pending_approval: 24h (owner needs time to respond)
    // pending_payment:  slotHoldMinutes (default 60 min)
    ...
    const { data: booking, error: insertError } = await supabase
      ...
      .insert({
        ...
        end_time,
        location_type,
        house_call_address:  location_type === 'house_call'
                               ? house_call_address!.trim()
                               : null,
    ...
    const formattedDate = new Date(booking_date + 'T00:00:00').toLocaleDateString('en-KE', {
    ...
    const emailData: NewBookingEmailData = {
      ...
      artistName:       artist?.name ?? 'Owner assigns',
      bookingDate:      formattedDate,
      startTime:        formattedTime,
      locationType:     location_type,
      houseCallAddress: house_call_address?.trim(),
      servicePrice:     servicePrice,
      travelFee:        houseTravelFee,
    ...
    sendEmail({
      to:      OWNER_EMAIL,
      subject: isLateNight
        ? `🌙 Late Night Request — ${customer_name} (${service.name})`
        : `✦ New Booking — ${customer_name} (${service.name})`,
